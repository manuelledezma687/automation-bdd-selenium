...
class BaseActions:
      ...
      def element_click(self,by_locator):
            try:  
                  WebDriverWait(self.browser,timeout=10).until(EC.visibility_of_element_located(by_locator))
      ...
      def is_enabled(self,by_locator):
            try:
                  self.browser.find_element(*by_locator)
                  WebDriverWait(self.browser,timeout=10).until(EC.element_to_be_clickable(by_locator))
            except EX as e:
                  print("Exception! Can't see the element")
            assert True,"Test pass"